...
class NeuralNet:
    def __init__(self, dataFile, header=True, h=4):
        ...
        raw_input = pd.read_csv(dataFile, sep= ",") 
        # TODO: Remember to implement the preprocess method -- DONE
        ...
        #----Edit-----
        self.testX = self.test_dataset.iloc[:, 0:(ncols -1)].values.reshape(nrows, ncols-1)
        self.testy = self.test_dataset.iloc[:, (ncols-1)].values.reshape(nrows, 1)
        #----Edit-----
    ...
    #
    # TODO: Define the derivative function for tanh, ReLu and their derivatives -- DONE
    #
    ...
    def forward_pass(self, activation="sigmoid"):
        ...
        # TODO: I have coded the sigmoid activation, you have to do the rest -- DONE
        if activation == "sigmoid":
            self.X_hidden = self.__sigmoid(in_hidden)
        else if activation == "tanh":
            self.X_hidden = self.__tanh(in_hidden)
        else:
            self.X_hidden = self.__relu(X_hidden)
...
if __name__ == "__main__":
    ...
    #preprocessData()
    input = "https://raw.githubusercontent.com/chiky330/Metro_Interest_Data/master/Metro_Interest_Data.csv"
    neural_network = NeuralNet(input)
